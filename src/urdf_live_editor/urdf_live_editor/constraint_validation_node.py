...
def main(args=None):
    """Start the constraint validation node and spin until shutdown."""
    import rclpy
    from rclpy.node import Node

    rclpy.init(args=args)
    node = Node('constraint_validation_node')
    node.get_logger().info(
        'constraint_validation_node started (Milestone 0 skeleton).')
    try:
        rclpy.spin(node)
